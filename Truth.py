...
class Path:
    ...
    def get_truthvalue(self):
        """Gets the truth value of the path. The numbers are based on the values of ``self.nodes``. 
        """
        truthvalue = 1
        for idx, x in enumerate(self.nodes):
            if idx == 0:
                continue    # ignore the first two connections (following Ciampaglia's paper since it would yield to true.)
            
            errorlog(x.backlinkcount)
            truthvalue += math.log(x.backlinkcount,10)
